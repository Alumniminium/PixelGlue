...
namespace Pixel.Entities
{
    ...
    {
        private static readonly Game _engine;
        ...
        static Camera()
        {
            foreach (var p in typeof(Game).GetProperties(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Static))
                if (p.GetValue(_engine) is Game g)
                    _engine = g;
            _graphicsDevice = _engine.GraphicsDevice;
        ...
        private float _angle,
            _rotCos,
            _rotSin;
        private Vector3 _xyz = new Vector3(0, 0, 1);
        private Matrix _projectionMatrix,
            _originMatrix;
        private (int Width, int Height) _viewportRes,
             _virtualRes;
        private bool _hasVirtualRes,
            _angleDirty;
        ...
        public Matrix ScaleMatrix(float z = 0)
        {
            ...
            var zoomFromZ = ScaleFromZ(Z, z);
            matrix.M11 = Scale.X * VirtualScale * zoomFromZ;
            matrix.M22 = Scale.Y * VirtualScale * zoomFromZ;
            return matrix;
        }
        ...
        public Matrix View(float z = 0)
        {
            ...
            float zoomFromZ = ScaleFromZ(Z, z),
                scaleM11 = Scale.X * VirtualScale * zoomFromZ,
                scaleM22 = Scale.Y * VirtualScale * zoomFromZ,
                m41 = -_xy.X * scaleM11,
                m42 = -_xy.Y * scaleM22;
        ...
        private void UpdateBounds()
        {
            float x = -Origin.X,
                y = -Origin.Y,
                w = MathF.Ceiling(Origin.X / Scale.X) + .5f,
                h = MathF.Ceiling(Origin.Y / Scale.Y) + .5f,
                xcos = x * _rotCos,
                ycos = y * _rotCos,
                xsin = x * _rotSin,
                ysin = y * _rotSin,
